...
regret_log=zeros(iteration_total,n_trials);% Regret for each trial
...
for iteration=1:iteration_total
	...
	KL_UCB_est=zeros(1,n_arms);% Estimation of UCB for eac arm
...
for ii = (n_arms+1):n_trials
    for x=1:n_arms
%      KL_UCB_est(x)=mu_est(x)+sqrt(343*log(ii)/played(x));% Calculate UCB for each arm
       mu_est_var=mu_est(x)+0.00001;
                ...
                for it=1:50
                    mul=log(ii*(log(ii))^2+1)/(played(x))-(mu_est(x)*log(mu_est(x)/mu_est_var)+(1-mu_est(x))*log((1-mu_est(x))/(1-mu_est_var)));
                ...
                %disp(mu_P_est_var);
                KL_UCB_est(x)=mu_est_var;
    end
    [KL_UCB_max,arm_selected]=max(KL_UCB_est);% Index of max UCB arm
    ...
    regret_log(iteration,ii)=regret(iteration,ii)/log(ii);
